...
export default async function responseYesPage({ params }) {
  ...
  const response = await fetch(`https://useless-facts-a2df8-default-rtdb.firebaseio.com/responseYes.json?orderBy="userID"&equalTo="${params.userID}"&cacheBuster=${cacheBuster}`);
  // pass cacheBuster as a query parameter to the URL. this will make the URL unique every time the page is loaded, so the browser will not cache the data.
